...
public class ORCADLDataHandler implements CommandExecutor, Listener {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        // 检查参数数量是否正确
        if (args.length != 3) {
            sender.sendMessage("用法错误：/command <collectdata> <player> <start|stop>");
            return false;
        }
        ...
        // 检查玩家是否存在
        if (player == null) {
            sender.sendMessage("错误：玩家 '" + playerName + "' 未找到。");
            return true;
        }
        ...
        switch (subCommand) {
            case "collectdata":
                if ("start".equals(action)) {
                    if (collectors.containsKey(playerName)) {
                        sender.sendMessage("数据收集已经在进行中。");
                        return true;
                    }
                    ...
                    if (!collectors.containsKey(playerName)) {
                        sender.sendMessage("数据收集尚未开始。");
                        return true;
                    }
                    stopCollectingData(playerName);
                    sender.sendMessage("停止收集 " + playerName + " 的数据.");
                } else {
                    sender.sendMessage("无效的操作：请使用 'start' 或 'stop'。");
                    return false;
                }
                break;
            default:
                sender.sendMessage("无效的子命令。请使用 'collectdata'。");
                return false;
        }
        return true;
    }
    ...
    // 启动数据收集
    public void startCollectingData(String playerName) {
    ...
    // 停止数据收集
    public void stopCollectingData(String playerName) {
    ...
    private class DataCollector {
        ...
        public DataCollector(String playerName) {
            ...
            this.dataFile = new File(Bukkit.getPluginManager().getPlugin("ORCA").getDataFolder(), "data-" + startTime + ".csv");
            try {
